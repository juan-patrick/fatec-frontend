...
import { Card, CardHeader, CardContent, Grid, Typography, Divider, TextField, FormControl, FormLabel, RadioGroup, FormControlLabel, Radio, CardActions, Button } from '@material-ui/core';
...
export default function CreateCurso() {
  ...
  const handleSubmit = async (values, resetForm) => {
    await api.post('/cursoextensao', values).then(({ data }) => {
      history.push('/cursos');
    }).catch(error => {
      resetForm();
      console.log(error);
      ...
      <Grid container spacing={2}>
        <Grid item container spacing={2}>
          <Grid item md={8} xs={12}>
            <Typography variant="h5" gutterBottom>Cadastro de Cursos</Typography>
          </Grid>
          <Grid item md={7} xs={12}>
            <Card>
              <CardHeader title={<Typography variant="h6">Formulário de Cadastro de Cursos</Typography>} />
              <Divider />
              <Formik initialValues={{ nome_extensao: '', tipo_extensao: '', status: true }} validationSchema={CursoExtesaoSchema} onSubmit={(values, { resetForm }) => {
