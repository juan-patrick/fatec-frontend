...
export default function TableCursoExtensao({ data, setData }) {
  ...
  const deleteCurso = async (cursoId) => {
    await api.delete(`/cursoextensao/${cursoId}`).then(({ data }) => {
      console.log(data);
      setData();
      handleCloseDialog();
    }).catch(error => {
  ...
  return (
    <TableContainer component={Paper}>
      <Dialog open={open} disableBackdropClick disableEscapeKeyDown style={{ maxHeight: '90%' }}>
        <DialogTitle>
          Você deseja realmente deletar este curso?
        </DialogTitle>
        <DialogContent dividers>
          ...
          <DialogContentText>
            <b>O curso que será removido é: {cursoToDelete.nome_extensao || ''}</b>
          </DialogContentText>
          <DialogActions>
            <Grid container item xs={12} md={12} spacing={4} justify="flex-end">
              ...
              <Grid item>
                <Button color="primary" className={classes.buttonDelete} onClick={e => deleteCurso(cursoToDelete.id)}>
                  Excluir
                </Button>
              </Grid>
            </Grid>
            ...
            <TableRow key={curso.id}>
              ...
              <TableCell>
                <Button variant="contained" color="primary"
                  onClick={e => history.push(`/update/cursos/${curso.id}`)}>Alterar</Button>
              </TableCell>
              <TableCell>
                <Button variant="contained" color="secondary" onClick={e => handleOpenDialog(curso)}>Deletar</Button>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
